...
export default function LogIn() {
    ...
    if (email.length && !emailError && password.length) {
      ...
      handleAPIRequest("post", "login", { email: email, password: password })
        .then((response) => {
          if (response !== "invalid") {
            if (
              response?.user?.type === "bus" &&
              response?.user?.verified === "no"
            ) {
              setLoading(false);
              ...
              navigate("/");
              setLoading(false);
              // here we will redirect to  sign up page

              return;
            }

            if (response?.user.status != "active") {
              if (response?.user.status === "registration") {
                localStorage.setItem("token", response?.user?.token);
                dispatch(setUser(response.user));
                dispatch(setIsLoggedIn(true));
                setLoading(false);
  ...
  const handleMoreInformation = (value) => {
    let payload = {
      ...
      status: "active",
      address: true,
      license: user.type == "pro" ? true : false,
